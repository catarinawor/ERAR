...
ShakerMethod1 <- function(D,M){
    ...
    TempTerminalLegalDropoffs <- matrix(0,nrow=length(allBY), ncol= D$MaxAge)
    TempSublegalShakerDropoffs <- matrix(0,nrow=length(allBY), ncol= D$MaxAge)

    #TotalTerminalShakers <- matrix(0,nrow=length(allBY),ncol=D$MaxAge)
    #TotalTerminalShakerDropoffs <- matrix(0,nrow=length(allBY),ncol=D$MaxAge)
